...
const loginMiddleware = store => next => (action) => {
  ...
  const userId = get(currentState, ['auth', 'user', 'data', '_id'], false)
  ...
  switch (action.type) {
    ...
    case REHYDRATE:
      if (window.location.pathname === '/auth/signup') {
        //
      } else if ((userId === false || isLogin === false)
    ...
    case actions.SUBMIT_LOGIN:
      ...
      authAPI.submitLogin(
        ...
        .then((res) => {
          if (res.success === true) {
            store.dispatch(actions.submitLoginSuccess(res.payload, action.payload.redirectTo))
          } else {
